...
const { dbModel } = require('../../db_model/hcp/administrativeFee.js');

async function list(user) {
	console.log('DB:administrativeFee.list(user)');
	let query = ``;
	...
	query = `SELECT ${fieldList} FROM ${dbModel.tableName} A
            ...
            ORDER BY [AdministrativeFeeName]`;
...
async function get(user, id) {
	console.log('DB:administrativeFee.get(user, id)');
	console.log(`id: ${id}`);
...
async function update(user, id, data) {
	console.log('DB:administrativeFee.update(user, id, data)');
...
async function insert(user, data) {
	console.log('DB:administrativeFee.insert(user, data)');
...
async function remove(user, id) {
	console.log('DB:administrativeFee.remove(user, id)');
