...
const { dbModel } = require('../../db_model/hcp/clientContribution.js');
const { utcDate2dbDate } = require('../../utils/dateTimeUtil.js');

async function list(user, queryParams) {
	console.log('DB:clientContribution.list(user)');
	let query = ``;
	...
	query = `SELECT 
            ...
            JOIN ClientContributionType C ON C.[ContributionTypeID] = A.[ContributionTypeID] `;

	let where = `WHERE B.[CompanyID] = ${user.companyId} `;

	// suppress $0.00 Amount records
	where += 'AND [Amount] != 0 ';
	...
	query += `ORDER BY C.[ContributionTypeName]`;
...
async function get(user, id) {
	console.log('DB:clientContribution.get(user, id)');
	console.log(`id: ${id}`);
...
async function update(user, id, data) {
	console.log('DB:clientContribution.update(user, id, data)');
...
async function insert(user, data) {
	console.log('DB:clientContribution.insert(user, data)');
	...
	stmt += `UPDATE ${dbModel.tableName} 
			...
			AND [ContributionTypeID] = ${data.contributionTypeId} 
			AND [EndDate] IS NULL;\n`;
	/*stmt += `UPDATE ${dbModel.tableName} 
...
async function remove(user, id) {
	console.log('DB:clientContribution.remove(user, id)');
...
async function amount(user, clientId, date, contributionTypeId) {
	console.log('DB:clientContribution.amount(user, clientId, date, contributionTypeId)');

	let query = `SELECT
				CC.[Amount] AS amount
				FROM ClientContribution CC
				JOIN Clients C ON C.[ID] = CC.[ClientID] AND C.[CompanyID] = ${user.companyId}
				WHERE CC.[ClientID] = ${clientId}
				AND '${date}' >= CC.[StartDate] AND ('${date}' <= CC.[EndDate] OR CC.[EndDate] IS NULL)
				AND CC.[ContributionTypeID] = ${contributionTypeId}`;
