...
const { dbModel } = require('../../db_model/hcp/clientInitialExpenditure.js');

async function list(user, queryParams) {
	console.log('DB:clientInitialExpenditure.list(user, queryParams)');
	let query = ``;
	...
	query = `SELECT 
            ...
            JOIN [OutboundType] C ON C.[OutboundID] = A.[OutboundID] `;
	...
	query += `ORDER BY B.[Last Name], B.[First Name], C.[OutboundName]`;
...
async function get(user, id) {
	console.log('DB:clientInitialExpenditure.get(user, id)');
	console.log(`id: ${id}`);
...
async function update(user, id, data) {
	console.log('DB:clientInitialExpenditure.update(user, id, data)');
...
async function insert(user, data) {
	console.log('DB:clientInitialExpenditure.insert(user, data)');
...
async function remove(user, id) {
	console.log('DB:clientInitialExpenditure.remove(user, id)');
